"""
train_empirical_llr.py - Fine-tune Neural-MS decoder tren EMPIRICAL-LLR
"""
...
from wifakey_module.wifakey_lib.utils import lssc_binary as _original_lssc_binary
...
class DummySession:
    """Lớp Session giả lập giúp WiFaKeyHandler khởi tạo cực nhẹ mà không chiếm tài nguyên TF."""
...
V1_FINETUNED_WEIGHTS_PATH = os.path.join(
    os.path.dirname(__file__), "weights", "Weights_Var_MS_finetuned"
)
V1_FINETUNED_BIASES_PATH = os.path.join(
...
# dung nhu thiet ke goc (truoc khi giam vi may yeu). Ly do can:
# batch_size=4 qua nho -> gradient tung buoc rat nhieu -> co the
...
def build_trainable_decoder(
    ...
    with graph.as_default():
        ...
        # GRADIENT ACCUMULATION: thay vi optimizer.minimize() (1 buoc = 1 lan
        # forward+backward+apply), tach thanh 3 phan de train loop tu tich luy
        # grad qua nhieu micro-batch truoc khi apply 1 lan (xem ACCUM_STEPS o
        # tren) - KHONG tang RAM/VRAM dinh vi moi micro-batch van chi BATCH_SIZE
        # mau nhu cu, chi khac la khong apply ngay sau moi micro-batch.
        grads_and_vars = optimizer.compute_gradients(loss, var_list=trainable_vars)
        grad_by_var_name = {v.name: g for g, v in grads_and_vars}
        grad_fetch_tensors = []  # dung de sess.run() lay gia tri grad tung micro-batch
        grad_placeholders = []  # dung de feed grad DA TRUNG BINH vao khi apply
        apply_pairs = []
        for v in trainable_vars:
            g = grad_by_var_name[v.name]
            assert (
                g is not None
            ), f"Bien {v.name} khong co gradient - kiem tra lai loss."
            grad_fetch_tensors.append(g)
            ph = tf.placeholder(
                tf.float32, shape=v.shape, name=f"gradph_{v.name.split(':')[0]}"
            )
...
def train_one_config(
    ...
    if verbose:
        print(f"\n{'='*66}\n[{config_name}] khoi tao tu: {init_weights_path}\n{'='*66}")
        print(
    ...
    with tf.Session(graph=tg["graph"], config=config) as sess:
        ...
        baseline_loss, baseline_bit, baseline_exact = evaluate_val_chunked(
            sess, tg, val_llr, val_target, BATCH_SIZE
        )
        history.append(
            {
                "epoch": 0,
        ...
        for epoch in range(n_epochs):
            ...
            for _ in range(steps_per_epoch):
                # Tich luy gradient qua ACCUM_STEPS micro-batch (moi micro-batch
                # van chi BATCH_SIZE mau -> RAM/VRAM dinh KHONG doi), roi apply
                # trung binh 1 lan -> effective batch = BATCH_SIZE*ACCUM_STEPS.
                accumulated = [
                ...
                for _ in range(ACCUM_STEPS):
                    ...
                    fetches = tg["grad_fetch_tensors"] + [tg["loss"], tg["bit_correct"]]
                    *grad_vals, loss_val, acc_val = sess.run(
                        fetches,
                        feed_dict={tg["xa"]: llr_batch, tg["target"]: target_batch},
                    )
                    for k in range(len(accumulated)):
                        accumulated[k] += grad_vals[k]
                    train_loss_list.append(loss_val)
                    train_acc_list.append(acc_val)

                feed = {
                    ph: accumulated[k] / ACCUM_STEPS
                    for k, ph in enumerate(tg["grad_placeholders"])
                }
                sess.run(tg["apply_op"], feed_dict=feed)
...
def main():
    print("Self-check quantizer per-bit truoc khi train (bat buoc)...")
    _selftest_against_original(_original_lssc_binary)

    print("Khởi tạo WiFaKeyHandler...")
    orig_session = tf.Session
    tf.Session = DummySession
    try:
    ...
    modulation = EmpiricalLLR(lookup_path=LOOKUP_PATH, masked_mag=MASKED_MAG)
    train_pairs, val_pairs = load_tune_genuine_pairs()

    configs = [
        dict(
            config_name="from_scratch",
            init_weights_path=ORIGINAL_WEIGHTS_PATH,
            init_biases_path=ORIGINAL_BIASES_PATH,
            output_weights_path=os.path.join(
                WEIGHTS_ROOT, "Weights_Var_MS_finetuned_v2_from_scratch"
            ),
            output_biases_path=os.path.join(
                WEIGHTS_ROOT, "Biases_Var_MS_finetuned_v2_from_scratch"
            ),
        ),
    ]

    if os.path.isdir(V1_FINETUNED_WEIGHTS_PATH) and os.path.isdir(
        V1_FINETUNED_BIASES_PATH
    ):
        configs.append(
            dict(
                config_name="from_v1_finetuned",
                init_weights_path=V1_FINETUNED_WEIGHTS_PATH,
                init_biases_path=V1_FINETUNED_BIASES_PATH,
                output_weights_path=os.path.join(
                    WEIGHTS_ROOT, "Weights_Var_MS_finetuned_v2_from_v1"
                ),
                output_biases_path=os.path.join(
                    WEIGHTS_ROOT, "Biases_Var_MS_finetuned_v2_from_v1"
                ),
            )
        )
    else:
        print(
            f"\n[canh bao] Khong tim thay checkpoint soft-LLR v1 tai "
            f"{V1_FINETUNED_WEIGHTS_PATH} -> BO QUA config 'from_v1_finetuned', "
            f"chi chay 'from_scratch'."
        )
    ...
    for cfg in configs:
        best_val_exact, best_epoch, history = train_one_config(
            cfg["config_name"],
            cfg["init_weights_path"],
            cfg["init_biases_path"],
            cfg["output_weights_path"],
            cfg["output_biases_path"],
            handler,
            encoder,
            modulation,
            train_pairs,
        ...
        results.append(
            dict(
                ...
                baseline_exact=history[0]["val_exact_match"],
                best_val_exact=best_val_exact,
                best_epoch=best_epoch,
                output_weights_path=(
                    cfg["output_weights_path"] if best_epoch > 0 else None
                ),
            )
        )

    print("\n" + "=" * 66)
    print("SO SANH KET QUA (buoc 4 - empirical-LLR)")
    print("=" * 66)
    for r in results:
        saved_note = (
            r["output_weights_path"]
            or "(KHONG cai thien so baseline - khong co gi duoc luu)"
        )
        print(
            f"[{r['config_name']:20s}] baseline(epoch0)={r['baseline_exact']:.4f}  "
            f"best={r['best_val_exact']:.4f} (epoch {r['best_epoch']})  -> {saved_note}"
        )
    if results:
        winner = max(results, key=lambda r: r["best_val_exact"])
        print(
            f"\n-> Config tot nhat theo val_exact_match: {winner['config_name']} "
            f"({winner['best_val_exact']:.4f})"
        )
        print("   (Trong so goc va checkpoint v1 cu khong he bi ghi de)")
        print(
            "   Buoc tiep theo: danh gia config thang tren tap 'select' "
            "(experiments/run_ab_soft_llr.py hoac ban tuong duong), "
            "so voi hard-BPSK va soft-LLR v1 cu. select/final chi dung MOT LAN."
        )
