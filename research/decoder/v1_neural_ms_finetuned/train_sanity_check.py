...
BẢN SỬA LẦN NÀY (tối ưu VRAM cho GPU yếu, KHÔNG đổi logic thuật toán):
  - Thay `handler = WiFaKeyHandler()` (khởi tạo TOÀN BỘ TF session + decoder
    Neural-MS 25 vòng lặp, chỉ để lấy M_matrix/intervals/_binarize_full())
    bằng `LightweightBinarizer` — chỉ load 2 file .npy (M_matrix, intervals),
    KHÔNG dựng TF graph nào cả. Loại bỏ hẳn 1 decoder session thừa chạy song
    song với decoder đang train (build_trainable_decoder) — tiết kiệm đáng
    kể VRAM trên GPU 2GB.
  - Thêm M_MATRIX_OVERRIDE_PATH (mặc định None = dùng M_matrix.npy gốc) để
    tái dùng file này cho bước retrain decoder trên M_matrix_oracle_lda sau
    khi đã xác nhận retrain với M_matrix gốc hoạt động đúng.

(Các sửa đổi thuật toán trước đó — selection-puncturing, per-bit confidence,
EmpiricalLLR — giữ NGUYÊN, không đổi gì.)
...
from wifakey_module.wifakey_lib import Encode, utils
...
WIFAKEY_DATA_PATH = os.path.join(_PROJECT_ROOT, "wifakey_module", "data")
# None = dùng M_matrix.npy gốc trong WIFAKEY_DATA_PATH. Sau khi retrain với
# M_matrix gốc đã xác nhận hoạt động đúng (FRR về mức hợp lý), đổi dòng này
# thành đường dẫn M_matrix_oracle_lda_regeps*.npy để retrain decoder riêng
# cho M_matrix mới -- và NHỚ đổi OUTPUT_WEIGHTS_PATH/OUTPUT_BIASES_PATH sang
# thư mục khác (vd "..._finetuned_oracle_lda") để không ghi đè bản retrain
# cho M_matrix gốc.
M_MATRIX_OVERRIDE_PATH = None
...
class LightweightBinarizer:
    """Chỉ load M_matrix + binarization_intervals để binarize embedding —
    KHÔNG khởi tạo TF session/decoder Neural-MS nào cả. Việc sinh dữ liệu
    huấn luyện (_encode_pair) chỉ cần binarize + M_matrix + LDPC encode,
    không cần chạy inference qua decoder gốc -- dùng WiFaKeyHandler() đầy
    đủ ở đây chỉ tốn thêm 1 TF session/decoder thừa trên GPU, không phục
    vụ mục đích gì trong file này.
    """

    def __init__(self, data_path=WIFAKEY_DATA_PATH, m_matrix_override_path=None):
        matrix_path = m_matrix_override_path or os.path.join(data_path, "M_matrix.npy")
        self.M_matrix = np.load(matrix_path)

        interval_path = os.path.join(data_path, "binarization_intervals.npy")
        ...
        self.feature_length = N * Z  # 832 -- khớp WiFaKeyHandler.feature_length
        self.key_length = (N - m) * Z  # 160 -- khớp WiFaKeyHandler.key_length

        if self.full_binary_length <= self.feature_length:
            raise ValueError(
                f"full_binary_length ({self.full_binary_length}) phải LỚN HƠN "
                f"feature_length ({self.feature_length}) để selection-puncturing "
                f"có ý nghĩa -- khớp yêu cầu của SecureWiFaKeyHandler."
            )
    ...
    def _binarize_full(self, feature_vector_float: np.ndarray) -> np.ndarray:
        ...
        return utils.lssc_binary(feature_vector_2d, interval=self.intervals).flatten()
...
def _encode_pair(handler, encoder, modulation, emb_enroll, emb_verify, rng):
    # --- Enroll: SELECTION-PUNCTURING, khop dung SecureWiFaKeyHandler dang
    # dung trong benchmark that -- moi bit trong feature_length vi tri chon
    # deu la bit sinh trac THAT, khong co bit nao bi ep ve hang so.
    ...
    # --- Verify: margin PER-BIT dung chuan, khop dung dinh nghia da dung
    # de fit reliability_lookup.npz.
...
def train_one_config(
    ...
    handler = LightweightBinarizer(m_matrix_override_path=M_MATRIX_OVERRIDE_PATH)
    ...
    if verbose:
        ...
        print(
            f"M_matrix: {'GOC' if M_MATRIX_OVERRIDE_PATH is None else M_MATRIX_OVERRIDE_PATH}"
        )
