...
MỤC ĐÍCH
--------
[ĐÃ CHUYỂN SANG SecureWiFaKeyHandler — selection-puncturing, đã vá lỗ hổng
AND-mask-về-0. Baseline ở đây nghĩa là "SecureWiFaKeyHandler + M_matrix gốc",
KHÔNG còn là WiFaKeyHandler (mask cũ) như các lần chạy trước. Kết quả lần
này KHÔNG so sánh trực tiếp được với kết quả cũ trong oracle_lda_results/ —
2 kiến trúc chọn bit khác nhau hoàn toàn (cắt cố định 277 chiều đầu vs chọn
ngẫu nhiên trải đều 512 chiều).]
...
qua binarize + selection-puncturing + LDPC decode (decoder KHÔNG retrain,
vẫn dùng trọng số train trên phân phối bit cũ) hay không?"
...
RESULTS_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "oracle_lda_results_SECURE"
)
...
def run_one_handler(which: str):
    ...
    print(
        f"\n[3/4] Khởi tạo handler '{which}' (SecureWiFaKeyHandler — selection-puncturing)..."
    )
    sys.path.insert(0, os.path.join(PROJECT_ROOT, "research", "commitment"))
    if which == "baseline":
        from research.commitment.v1_selection_puncturing import SecureWiFaKeyHandler

        handler = SecureWiFaKeyHandler()
    else:
        from research.commitment.wifakey_handler_secure_lda_variant import SecureWiFaKeyHandlerLDAVariant

        handler = SecureWiFaKeyHandlerLDAVariant(m_matrix_override_path=NEW_MATRIX_PATH)
