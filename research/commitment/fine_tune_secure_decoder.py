"""
fine_tune_secure_decoder.py

Fine‑tune Neural‑MS decoder từ trọng số GỐC, chỉ huấn luyện 20 vòng cuối.
Dùng dữ liệu từ pipeline AN TOÀN (SecureWiFaKeyHandler + EmpiricalLLR).
Thiết kế để tránh OOM trên GPU 2GB (batch size 1 + gradient accumulation).
"""
...
N, m, Z = 52, 42, 16
ITERS_MAX = 25
TRAINABLE_ITERS = 20  # Chỉ huấn luyện 20 vòng cuối
FREEZE_UNTIL = ITERS_MAX - TRAINABLE_ITERS  # 5 vòng đầu bị đóng băng
...
OUTPUT_WEIGHTS_DIR = os.path.join(
    os.path.dirname(__file__), "weights", "Weights_Var_MS_finetuned_secure"
)
OUTPUT_BIASES_DIR = os.path.join(
    os.path.dirname(__file__), "weights", "Biases_Var_MS_finetuned_secure"
)
...
def build_finetune_graph():
    """Xây dựng đồ thị fine‑tune: đóng băng FREEZE_UNTIL vòng đầu, huấn luyện các vòng sau."""
    # Đọc base graph
    ...
    # ---- Xây dựng các ma trận cố định (giống hệt code gốc) ----
    ...
    with graph.as_default():
        ...
        for i in range(ITERS_MAX):
            ...
            # Đóng băng đầu ra của vòng cuối cùng bị đóng băng
            if i == FREEZE_UNTIL - 1:
